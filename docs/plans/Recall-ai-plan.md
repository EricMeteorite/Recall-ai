...
> - **Local 模式**：运行时内存约 500-600MB（推荐 2GB+ 可用内存）
> - **Lite 模式**：运行时内存约 80-100MB（适合低配电脑，禁用向量检索）
...
### 方式四：Lite 模式（低配电脑/内存不足）
```bash
pip install recall-ai
recall init --lite   # Lite 模式，内存占用仅 ~80MB (--lightweight 也可用)
recall chat
```

> 💡 Lite 模式禁用向量语义检索，但关键词匹配、伏笔追踪、规范检查等核心功能完全保留。
> 对于 90% 的使用场景，Lite 模式已经足够。
