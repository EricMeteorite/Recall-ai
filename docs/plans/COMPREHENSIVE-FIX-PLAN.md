...
| **配置模板同步** | ✅ 已完成 | start.ps1/start.sh/manage.ps1/manage.sh 模板已同步 |
| **前端配置界面** | ✅ 已完成 | ST 插件添加容量限制配置 UI |
| **归档文件索引重建** | ✅ 已完成 | v3 标记文件，支持索引 archive/*.jsonl |
...
| 归档文件无法语义检索 | ✅ 已解决 | v3 索引重建支持 archive/*.jsonl 文件 |
| 前端无法配置容量限制 | ✅ 已解决 | ST 插件新增容量限制配置面板 |
...
- 配置系统：环境变量 + 热更新
- 归档机制：伏笔/条件自动归档 + 分卷存储
- VectorIndex 索引：创建时索引 + 自动重建（含归档文件）
- 衰减公式修正：乘法衰减
- doc_id 格式统一
