...
> - ✅ **规范100%遵守**：属性/关系/时间线检测✅ + 绝对规则检测✅（2026-01-24完成）
...
| 5 | 规范100%遵守 | L0注入 + 规则编译 + 属性检查 | ✅ | L0注入✅ 属性检测✅ 绝对规则检测✅（2026-01-24完成） |
...
### ✅ 3. 规范100%遵守 - ✅ 已完成（2026-01-24 LLM语义检测）
...
| `absolute_rules` 存储 | ✅ | 用户可配置的绝对规则列表 |
| `get_injection_text()` | ✅ | 按场景(RP/coding)返回注入内容 |
| **⭐数值属性冲突检测** | ✅ | 年龄/身高/体重等（2026-01-24增强） |
| **⭐外貌属性冲突检测** | ✅ | 发色/眼色/肤色，支持颜色同义词合并 |
| **⭐状态冲突检测** | ✅ | 生死/婚姻状态，自动检测对立状态 |
| **⭐关系冲突检测** | ✅ | 朋友vs敌人等对立关系检测 |
| **⭐时间线完整检测** | ✅ | 年龄一致性、事件顺序、时态冲突 |
| **⭐否定句违反检测** | ✅ | "不会X"但做了X的逻辑错误检测 |
| **⭐生死矛盾检测** | ✅ | 角色死后行动的逻辑错误检测 |
| **⭐ 绝对规则LLM检测** | ✅ | **LLM语义理解，支持任意复杂规则！**（2026-01-24实现） |
| 警告历史记录 | � 可选 | 弹窗消失后无法查看历史警告（非核心功能） |

---

#### ✅ 绝对规则检测已实现（2026-01-24 - LLM语义版）

**核心改进**：使用 **LLM 进行语义理解**，不再是简单的关键词匹配！

**支持的规则格式**：
- ✅ **任意长度的自然语言描述**
- ✅ 「角色是一个温柔的女孩，她从不会对任何人发火，即使被欺负也会选择忍耐」
- ✅ 「角色说话时总是用敬语，从不直呼对方名字」
- ✅ 「禁止出现任何形式的暴力描写，包括但不限于打斗、伤害、威胁」
- ✅ 语义等价理解（"发火"="生气"="愤怒"）
- ✅ 隐含违反检测（规则是"温柔"，"狠狠地踢"就是违反）

**工作链路**：
```
用户添加规则（可以是一长段描述）
         ↓
存储到 CoreSettings.absolute_rules 
         ↓
API更新时同步到 ConsistencyChecker
         ↓
新消息到达 → check() → _check_rules_with_llm()
         ↓
LLM 分析内容是否违反任何规则（语义理解）
         ↓
检测到违规 → 返回详细说明 → 前端 toastr 弹窗
```

**技术实现**：
- `ConsistencyChecker` 接收 `llm_client` 参数
- `_check_rules_with_llm()` 构建检测 prompt，让 LLM 判断
- 无 LLM 时功能不生效（规则检测需要语义理解，关键词匹配会产生误报）
- 每次检测约消耗 ~500 tokens（成本极低）

**增强版 ConsistencyChecker 功能清单**（2026-01-24）：

```
┌─────────────────────────────────────────────────────────────┐
│              ConsistencyChecker v4.0 (LLM语义版)            │
├─────────────────────────────────────────────────────────────┤
│  属性检测（15+种）:                                          │
│  ├─ 数值属性: 年龄、身高、体重                               │
│  ├─ 外貌属性: 发色、眼色、肤色（支持颜色同义词合并）         │
│  ├─ 固定属性: 血型、性别、种族                               │
│  └─ 状态属性: 生死状态、婚姻状态、职业、位置                 │
├─────────────────────────────────────────────────────────────┤
│  关系检测:                                                   │
│  ├─ 对立关系词典: 朋友↔敌人, 恋人↔仇人, 信任↔背叛           │
│  └─ 双向关系存储: A和B的关系 = B和A的关系                   │
├─────────────────────────────────────────────────────────────┤
│  时间线检测（完整版）:                                       │
│  ├─ 时态冲突: 同一日期的过去/将来描述矛盾                   │
│  ├─ 年龄一致性: "N年前X岁" → 现在应该是X+N岁               │
│  ├─ 事件顺序: "A在B之前" vs "B在A之前"                      │
│  └─ 生死矛盾: 角色死亡后不能有新动作                        │
├─────────────────────────────────────────────────────────────┤
│  否定句检测:                                                 │
...
│  └─ 违反: 声称"不会X"但后来"做了X"                         │
├─────────────────────────────────────────────────────────────┤
│  ⭐ 绝对规则LLM检测（2026-01-24新增）:                       │
│  ├─ LLM语义理解: 支持任意长度的自然语言规则描述              │
│  ├─ 语义等价: "发火"="生气"="愤怒" 自动理解                 │
│  ├─ 隐含违反: 规则"温柔" → 检测"狠狠地踢"                  │
│  └─ 依赖LLM: 无LLM时功能不生效（需配置LLM_API_KEY）         │
├─────────────────────────────────────────────────────────────┤
│  颜色同义词合并:                                             │
│  ├─ black: 黑色/乌黑/漆黑/墨黑                              │
│  ├─ blonde: 金色/金黄/淡金/亚麻色                           │
...
**对 RP 场景的影响**：
- ✅ 完整的属性冲突检测，避免"黑发"突然变"金发"
- ✅ 关系一致性检查，避免"朋友"突然变"敌人"
- ✅ 生死矛盾检测，避免"死掉的角色继续行动"
- ✅ 年龄时间线检查，确保年龄变化符合时间流逝
- ✅ **用户自定义规则 LLM 语义检测**，支持任意复杂的规则描述！
...
当前项目已实现 **96%** 的功能（27/28项 ✅），核心架构完整。

### 功能统计

| 分组 | 项目数 | 完成数 | 状态 |
|------|:------:|:------:|:----:|
| 第一组：核心功能需求 | 15 | 14✅ + 1❌ | 93% |
...
| 功能 | 状态 | 说明 | 工作量 | 优先级 |
|------|:----:|------|:------:|:------:|
| CodeIndexer | ❌ 未实现 | 整个模块（代码场景专用，RP可选） | 2-3天 | 🟢 低 |

### 可选增强项（非必须）

| 功能 | 状态 | 说明 | 工作量 | 优先级 |
|------|:----:|------|:------:|:------:|
| 警告历史记录 | � 可选 | toastr消失后无法查看历史 | 0.5天 | 🟢 低 |
| 测试规则功能 | 🟡 可选 | 验证规则是否生效的UI | 1天 | 🟢 低 |
...
| 规范100%遵守 | ✅ | **全部实现**（2026-01-24）<br>自动属性检测✅ + 用户规则检测✅ |
...
**已实现（用户配置的绝对规则 - LLM语义检测）**（2026-01-24新增）：
- ✅ 用户在前端「⚠️ 规则」添加的绝对规则**现已生效**
- ✅ 使用 **LLM 语义理解**，支持任意复杂的自然语言规则
- ✅ 例：「角色温柔善良」→ LLM检测"狠狠地踢"属于违反
- ✅ 例：「故事背景是古代」→ LLM检测"手机/汽车"属于违反
- ⚠️ **注意：需要配置 LLM_API_KEY 才能生效**（无LLM时功能不生效）
